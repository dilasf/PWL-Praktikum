                ...
                <div class="p-6 text-black dark:text-black">
                    <x-table>
                        ...
                        <tr>
                            ...
                            <td>{{ $book->quantity }}</td>
                            <td>{{ $book->bookshelves->code }}-{{ $book->bookshelves->name }}</td>
                            <td>
                                <x-primary-button tag="a" href="{{ route('book.edit', ['id' => $book->id]) }}"> Edit </x-primary-button>
                                <x-danger-button
                                x-data=""
                                x-on:click.prevent="
                                    $dispatch('open-modal', 'confirm-book-deletion');
                                    $dispatch('set-action', '{{ route('book.destroy', $book->id) }}');">
                                {{ __('Delete') }}
                            </x-danger-button>
